...
The Observer pattern defines a one-to-many dependency between objects so that when one object changes state, all its dependents are notified automatically. Very popular in iOS (NotificationCenter, Combine, RxSwift).
...
- `NotificationCenter` for system and custom notifications
